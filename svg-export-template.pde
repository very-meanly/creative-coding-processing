...
// as many times per second as Processing can do,
// which is fun since you can use variables like
// frameCount (current frame) to make things change
...
void draw() {
  ...
  if (record) {
    ...
    // can also export SVGs, but it can't handle 3D, so
    // beginRaw can and is used here instead.
    beginRaw(SVG, "output.svg");

    // Helps improve geometry of 3D geometry drawn to 2D files
...
// When the mouse is clicked, we'll set to record to true so
